...
Tutorial {
    ...
    function initHowToCancelAGesture() {
        ...
        lesson.addStep(new Tutorial.Step({pointTo: backspaceKey,
                                          select: backspaceKey}));
        lesson.addStep(new Tutorial.Step({text: "... and selecting the \"Cancel\" action by looking at the button",
                                          pointTo: pEyeBackspaceWord,
                                          select: pEyeBackspaceWord}));
        lesson.addStep(new Tutorial.Step({text: "... and looking back at the " + backspaceKey.text + " key",
                                          pointTo: backspaceKey,
                                          click: pEyeBackspaceWord}));
    }
    ...
    function initHowToDeleteWord() {
        ...
        lesson.addStep(new Tutorial.Step({text: "Use the " + backspaceKey.text + " key to delete the last typed word",
                                          poinTo: backspaceKey,
                                          select: backspaceKey}));
        lesson.addStep(new Tutorial.Step({text: "Choose the desired action (\"" + pEyeBackspaceWord.text +  "\") by looking at it...",
                                          pointTo: pEyeBackspaceWord,
                                          select: pEyeBackspaceWord}));
    ...
    Timer {
        ...
        onTriggered: {
            ...
            case 2:
                gazeTimer.moveTo(key.centerX, key.centerY);
                pEyeGesture.click(pEyeGesture, new Date().valueOf() - tutorial.t0);
                keysPEyeMenu.hide();
                break;
            default:
                stop();

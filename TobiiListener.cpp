...
static const TX_STRING InteractorId = "EyeSwipeInteractor";
...
void OnFixationDataEvent(TX_HANDLE hFixationDataBehavior)
{
    ...
    if (txGetFixationDataEventParams(hFixationDataBehavior, &eventParams) == TX_RESULT_OK) {
        ...
        /*if (eventType == TX_FIXATIONDATAEVENTTYPE_BEGIN)
        {
            fixationData.clear();
        }*/
        ...
        {
            //fixationData.append(QPointF(eventParams.X, eventParams.Y));
            emit emitter->newGaze(SamplePoint(Timer::timestamp(), QPointF(eventParams.X, eventParams.Y)));
        }

        /*if (eventType == TX_FIXATIONDATAEVENTTYPE_END && fixationData.size() > 0)
        {
            float xSum = 0;
            float ySum = 0;
            int n = fixationData.size();
            foreach (QPointF sample, fixationData) {
                xSum += sample.x();
                ySum += sample.y();
            }
            emit emitter->newGaze(SamplePoint(Timer::timestamp(), QPointF(xSum/n, ySum/n)));
        }*/
    } else {
        qDebug() << "(EyeX) Failed to interpret gaze data event packet.";
    }
